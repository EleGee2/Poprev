...
const validateToken = async (req, res) => {
  try {
    ...
    const decoded = await promisify(jwt.verify)(token, process.env.JWT_SECRET).catch(() => {
      return {
        status: "failed",
        message: "You cannot be authenticated with this token"
      }
    })
...
const isUser = async (req, res, next) => {
  ...
  if(token.status === "failed") return res.status(400).json({ message: token.message})

  if (token.type === "user" || token.type === "admin") {
    req.user = token
    next()
  } else {
    return res.status(400).json({ message: "Access not granted to this user resource"})
  }
}
...
const isOrganisation = async (req, res, next) => {
  ...
  if (Object.values(OrganisationTypes).includes(token.type)) {
    ...
    return res.status(400).json({ message: "Access not granted to this organisation resource"})
  }
}
